...
class Trainer(SportsmenInGym):
    def __init__(self,name: str,state : bool,age: int,experience : int, price: int,free_seats: int) -> None:
    ...
    def registration (self) -> int:
        self.free_seats -= 1
        return  self.free_seats

    def add_seats (self,col) -> int:
        self.free_seats += col
        return self.free_seats
...
class EduSportsmen(SportsmenInGym):
    def __init__(self,name:str,state:bool,_membership_level:int) -> None:
        super().__init__(name,state)
        self.cnt_wokot = 1 
        self.discount = _membership_level * 0.1
    @property
    def cnt_wokot(self) -> int:
        return self._cnt_wokot
    @cnt_wokot.setter 
    def cnt_wokot(self, value) -> None:
        validate_cnt(value)
        self._cnt_wokot = value
    def registration (self,cnt) -> None:
        self.cnt_wokot += cnt
    def complited_workout(self) -> None:
        self.cnt_wokot -= 1
    def __str__(self) -> str:
